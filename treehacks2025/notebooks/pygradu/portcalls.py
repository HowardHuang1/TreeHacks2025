...
def reset_ais_for_portcalls(ais):
    ais = ais[['timestamp', 'mmsi', 'lat', 'lon',
               'speed', 'node', 'vessel_type', 'iceclass']]
...
def calculate_voyages(ais, ports):
    ...
    for mmsi, observations in vessels:
        ...
        prev.in_port = False
        observations['long_speed'] = observations.speed.iloc[::-
                                                             1].rolling(3).mean().iloc[::-1]
        observations['really_long_speed'] = observations.speed.iloc[::-
                                                                    1].rolling(5).mean().iloc[::-1]
        for i, obs in observations.iterrows():
            cursor += 1
            obs.in_port = obs.node in ports

            ais_fresh = pd.to_datetime(obs['timestamp']) <= (pd.to_datetime(
                prev['timestamp']) + datetime.timedelta(hours=time_limit_hours))
            ...
            if voyage['atd'] != not_available and obs.in_port \
                ...
                ais_portcalls = ais_portcalls.append(
                    df_voyage, ignore_index=True)
...
def get_voyage_lengths(ais):
    ...
    for i, voyage in voyages:
        ...
        travel_time = (pd.to_datetime(end.timestamp) -
                       pd.to_datetime(start.timestamp)).total_seconds() / 60.0
        ...
        rows.append([i, voyage_dist, start_end_dist,
                    difference, travel_time, mean_speed])
...
def get_voyage_total_accuracy(pred_voyages, actual_voyages):
    ...
    for i, pred_voyage in pred_voyages:
        ...
        pred_travel_time = (pd.to_datetime(pred_end.timestamp) -
                            pd.to_datetime(pred_start.timestamp)).total_seconds() / 60.0 / 60.0
        ...
        actual_travel_time = (pd.to_datetime(actual_end.timestamp) -
                              pd.to_datetime(actual_start.timestamp)).total_seconds() / 60.0 / 60.0
        ...
        ata_diff_minutes = abs((pd.to_datetime(
            actual_ata) - pd.to_datetime(predicted_ata)).total_seconds()) / 60.0
